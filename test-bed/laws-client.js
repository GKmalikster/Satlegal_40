...
const LAWS_DATABASE = [
  ...
  // FAMILY – SPECIAL MARRIAGE ACT (COURT MARRIAGE)
  {
    ...
    keywords: {
      ...
      strong: ['special marriage act','sma','court marriage','civil marriage','marriage officer','30-day notice','inter-faith marriage','inter-religious marriage','love marriage','registrar marriage'],
  ...
  // FAMILY – SENIOR CITIZENS MAINTENANCE & WELFARE
  {
    ...
    keywords: {
      ...
      weak: ['senior','parents','old age','mother','father','elderly','neglect']
    },
    sections: [],
    documents: [
  ...
  // CRIMINAL – NDPS (DRUG OFFENCES)
  {
    ...
    keywords: {
      exact: ['ndps case','drugs case','charas seized','ganja arrest','cocaine case','heroin seizure','commercial quantity ndps','bail under ndps','section 37 ndps','ncb arrest','mdma case','lsd seized','drug trafficking case','accused of drug trafficking','drug trafficking arrested','seized contraband drugs','drugs smuggling case','trafficking drugs defence','drug trafficking charge','ndps act accused','anticipatory bail in ndps case','commercial quantity narcotics section 37 ndps','ndps case section 37 twin conditions bail','drug peddling ndps section 20 cannabis','ndps section 20 cannabis case bail','accused of drug peddling ndps act'],
      strong: ['ndps','narcotic','psychotropic','commercial quantity','intermediate quantity','small quantity','ncb','fsl','sec 37 ndps','sec 50 ndps','sec 67 ndps','drug peddling','drug trafficking','contraband','drugs smuggled','drug supply chain','drug seizure','narcotics case'],
  ...
  // CRIMINAL – PMLA (MONEY LAUNDERING)
  {
    ...
    keywords: {
      ...
      strong: ['pmla','enforcement directorate','ecir','money laundering','predicate offence','provisional attachment','proceeds of crime','sec 45 pmla','fema violation','benami transaction','shell company fraud','hawala','layering placement integration','account frozen ed','ed freeze','ed froze','ed notice','pmla investigation','enforcement directorate action'],
  ...
  // CIVIL – NGT / ENVIRONMENT / POLLUTION
  {
    ...
    keywords: {
      ...
      strong: ['ngt','national green tribunal','pollution','pcb','cpcb','spcb','eia','environment protection','air pollution','water pollution','noise','crz','forest clearance','environmental damage','biodiversity'],
  ...
  // ── TAX – INCOME TAX / DIRECT TAX ──────────────────────────────────────────
  {
    ...
    keywords: {
      ...
      strong: ['income tax','IT notice','tax demand','tax scrutiny','TDS','tax deducted','tax refund','income tax return','ITR','capital gains','LTCG','STCG','assessment','reassessment','penalty income tax','search seizure','tax raid','benami','black money','undisclosed income','CIT(A)','ITAT','income tax tribunal','tax evasion','wealth tax','advance tax','self-assessment tax','26AS','form 16','tax officer','AO assessing officer','CBDT'],
  ...
  // ── INSURANCE – CLAIM DISPUTE ───────────────────────────────────────────────
  {
    ...
    keywords: {
      ...
      strong: ['insurance claim','claim rejected','claim denied','insurance dispute','policy','insurer refusing','cashless denied','insurance company','health insurance','life insurance','LIC','mediclaim','claim settlement','insurance ombudsman','IRDA','death benefit','sum assured','premium','policy lapse','surrender value','maturity amount','exclusion clause','pre-existing condition','waiting period','insurance fraud','insurance grievance','claim delay'],
...
function analyzeMultipleLaws(userInput, contextualAnswers = {}) {
  ...
  LAWS_DATABASE.forEach(law => {
    ...
    const hasChequeSignal = ['cheque','dishonour','bounce','sec 138','138 ni','ni act','nach','ecs bounce','pdc'].some(s => input.includes(s));
    ...
    const hasContractorFraudSignal = ['contractor did not complete','work not done','paid contractor','advance paid contractor','contractor took money','paid money contractor'].some(s => input.includes(s));
    ...
    if (isBnssBail && hasHabeasCorpusSignal) confidence = Math.min(confidence, 20);
    ...
    const hasRPwDEmployerSignal = ['wheelchair user not given ground floor accommodation','rpwd act violation employer wheelchair','wheelchair user ground floor accommodation rpwd','wheelchair user not given ground floor'].some(s => input.includes(s));
